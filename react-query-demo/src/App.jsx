...
const PostsContent = () => {
  ...
  const { 
    ...
    // staleTime is 10s, set in queryClient config below.
  });
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
      ...
      <span style={{ marginLeft: '15px', color: isFetching ? '#f97316' : '#10b981', fontWeight: 'bold' }}>
        {isFetching ? 'Background Updating...' : 'Data is ready and cached.'}
      </span>
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      // Keep data fresh for only 10 seconds to make the caching/refetch clearer
      staleTime: 1000 * 10, 
    },
  },
});
    ...
    <QueryClientProvider client={queryClient}>
      <div style={{ padding: '30px', fontFamily: 'Inter, Arial, sans-serif', backgroundColor: '#f0f4f8', minHeight: '100vh' }}>
        ...
        <p style={{ backgroundColor: '#fffbe3', padding: '15px', borderRadius: '8px', borderLeft: '5px solid #fcd34d', color: '#78350f' }}>
          **Caching Demo:** Click the toggle button. If you remount the component within 10 seconds, the data loads **instantly** from cache without a full network request, demonstrating optimal responsiveness.
        </p>
